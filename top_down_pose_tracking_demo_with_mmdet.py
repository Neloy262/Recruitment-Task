...
def main():
    ...
    for a,b,c,d in zip_longest(gen("1.mp4"),gen("2.mp4"),gen("3.mp4"),gen("4.mp4")):
        ...
        if b is  None:
            b = np.zeros([frame_height,frame_width,3],dtype=np.uint8)
        if c is  None:
            c = np.zeros([frame_height,frame_width,3],dtype=np.uint8)
        if d is  None:
            d = np.zeros([frame_height,frame_width,3],dtype=np.uint8)
